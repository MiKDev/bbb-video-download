...
const copyWebcamsVideo = async (input, output) => {
    childProcess.exec(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${input} -y ${ouput}`)
}

const copyWebcamsAudioToPresentation = async (presentation, webcams, output) => {
    childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${presentation.video} -i ${webcams.video} -c:v copy -c:a aac -map 0:0 -map 1:1 -shortest -y ${output}`)
}
...
const stackWebcamsToPresentation = async (presentation, webcams, output) => {
        ...
        childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${presentation.video} -i ${webcams.video} -filter_complex "[0:v]pad=width=${width}:height=${height}:color=white[p];[p][1:v]overlay=x=${presentation.width}:y=0[out]" -map [out] -map 1:1 -c:a aac -shortest -y ${output}`)
}

const addCaptions = async (captions, videoObject) => {
    const tmpFile = videoObject.video + '.tmp.mp4'

    let cmd = 'ffmpeg -hide_banner -loglevel error -threads 32 -i ' + videoObject.video
...
const renderFinalVideo = async (input, output) => {
    childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${input} -y ${output}`)
}
