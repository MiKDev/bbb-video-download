...
const combinedSlidesAndDeskshares = async (slides, deskshares, config, duration) => {
    ...
    childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${deskshares.video} -vf "scale=w=${width}:h=${height}:force_original_aspect_ratio=1,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:color=white" -c:v libx264 -preset ultrafast ${resizedDesksharesVideo}`)
    deskshares.parts.forEach((part, index) => {
        const presentationIn = (index == 0) ? slides.video : presentationOut
        childProcess.execSync(`ffmpeg -hide_banner -loglevel error -threads 32 -i ${presentationIn} -i ${resizedDesksharesVideo} -filter_complex "[0][1]overlay=x=0:y=0:enable='between(t,${part.start},${part.end})'[out]" -map [out] -c:a copy -c:v libx264 -preset ultrafast ${presentationTmp}`)
